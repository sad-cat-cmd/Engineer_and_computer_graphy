...
void Сhanging_Scale(int ratio) {
	for (int i = 0; i < vector_X_Points.size(); i++) { vector_X_Points[i] /= ratio; }
	for (int i = 0; i < vector_X_Points.size(); i++) { vector_Y_Points[i] /= ratio; }
}
...
void show_images(HDC _hdc) {
	// Создаём красный карандаш толщиной 1 пиксель
	HPEN hRedPen = CreatePen(PS_SOLID, 3, RGB(255, 0, 0));
	// Сохраняем старый карандаш, чтобы потом его вернуть
	...
	// Восстанавливаем предыдущий карандаш и удаляем созданный
